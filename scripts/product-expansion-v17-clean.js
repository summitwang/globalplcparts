...
function isBrandRootUrl(url, rule) {
  ...
  const cleanBrandUrl = String(rule.url || "")
    .toLowerCase()
    .replace(/\/+$/g, "");
...
function isValidIndustrialModel(model) {
  ...
  if (!/[0-9]/.test(upper) && upper.split("-").length <= 1 && upper.length > 18) {
    return false;
  }
...
async function extractProductDetail(page, productUrl, rule) {
  try {
    ...
    const fallbackModel = cleanModel(
      getRawModelFromUrl(productUrl),
      rule
    );

    const modelFromTitle = extractModelFromTitle(
      data.title,
      rule,
      fallbackModel
    );
    ...
    const cleanImages = data.images
      ...
      .filter(
        (x, index, arr) =>
          arr.findIndex((y) => y.url === x.url) === index
      )
      .map((x) => ({
        ...x,
        score: scoreImage(x),
  ...
  downloaded++;
      ...
      if (existing) {
        if (shouldReplaceImage(existing.image)) {
          ...
          skipped++;
          console.log(
            `[${rule.brand}] Skipped existing good image:`,
            model
          );
        }
      } else {
        products.push(
          makeProduct(rule, model, publicPath, productUrl, detail.desc)
        );
        added++;
...
async function main() {
  console.log("Product Expansion PRO MAX V17 CLEAN started");
  ...
  console.log("\n====================================");
  console.log("V17 CLEAN ALL BRANDS FINISHED");
